...
def safe_text(value):
    """Paragraph üçün mətni XML-safe edir (<, >, & kimi simvolları qoruyur)."""
    return escape(str(value))


def register_unicode_font():
    """
    Azərbaycan hərflərini (ə, ğ, ı, ş, ç, ö, ü) düzgün göstərmək üçün
    Unicode dəstəkli TTF font qeydiyyatdan keçirir.
    Uğurlu olarsa (font_name, bold_font_name) qaytarır, olmasa None, None.
    """
    candidates = [
        # Linux
...
class ReportGenerator:
    ...
    def create_report(self, profile, cleaning):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = os.path.join(
            self.output_dir, f"data_analysis_report_{timestamp}.pdf"
        )

        pdf = SimpleDocTemplate(path, pagesize=A4)
        styles = getSampleStyleSheet()

        # Azərbaycan hərfləri üçün Unicode font qeydiyyatı
        ...
        if font_name:
            ...
            print(
                "XƏBƏRDARLIQ: Unicode font tapılmadı. "
        ...
        page_width = A4[0] - 2 * 20 * mm  # sol/sağ marjinləri təxmini nəzərə alır
        ...
        for category, columns in profile["column_types"].items():
            ...
            content.append(
                Paragraph(
                    f"<b>{safe_text(category.upper())}</b>: {cols_text}",
                    styles["BodyText"],
                )
            )
        ...
        content.append(
            Paragraph(
                f"Təkrarlanan sətirlər: {safe_text(cleaning['duplicates'])}",
                styles["BodyText"],
            )
        )
        ...
        # Outliers (əvvəlki versiyada hesablanırdı, amma göstərilmirdi)
